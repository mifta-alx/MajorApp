<div>
    @include('livewire.majors.majormodal')
                                    ...
                                    @foreach ($majors as $major)
                                        <tr>
                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                {{ ($majors->currentPage() - 1) * $majors->perPage() + $loop->iteration }}
                                            </td>
                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                {{ $major->major }}
                                            </td>
                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                @php
                                                    $criteriaIds = json_decode($major->criteria_id, true);
                                                @endphp
                                                @foreach ($all_criterias as $criteria)
                                                    @if (in_array($criteria->criteria_id, $criteriaIds))
                                            ...
                                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                <div class="flex flex-row space-x-2">
                                                    <button wire:click="edit({{ $major->major_id }})"
                                                    ...
                                                    <button wire:click="delete({{ $major->major_id }})"
